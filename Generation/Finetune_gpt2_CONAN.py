...
def load_and_cache_examples(args, tokenizer, text,block_size,dataset="CONAN"):
    return preprocess.ConversationDataset(tokenizer, args, text,block_size, dataset)
...
params={
     'output_dir':'../HULK/Counterspeech/Saved_Models/Generator/',
...
if __name__ == "__main__":
    ...
    data = pd.read_json("../HULK/Counterspeech/Datasets/CONAN.json")
    
    # select only English sentences
    list_text = [x for i,x in enumerate(data.iloc[:,-1].values) if data["cn_id"][i][:2]=='EN']
    
    sentence_dict = {}
    for i in range(len(data)):
        if data["cn_id"][i][:2]=='EN':
            if data["hateSpeech"][i] in sentence_dict.keys():
                sentence_dict[data["hateSpeech"][i]].append(data["counterSpeech"][i])
            else:
                sentence_dict[data["hateSpeech"][i]] = [data["counterSpeech"][i]]
    
    def text_collect(dictionary):
        text = []
        for i, j in dictionary.items():
            for w in j:
                text.append(str(i + "<|endoftext|>" + w ))
        return text
    
    total_data_sentences = text_collect(sentence_dict)
